# -*- coding: utf-8 -*-
"""
Node-based tree structure for Excel template representation.

This module defines the hierarchical node structure used to represent
Excel templates as trees. Each element (workbook, sheet, row, cell, segment)
is represented as a node with enter/process/exit lifecycle.
"""
...
from .utils import tag_test, v_test, xv_test
...
class DebugInfo:
    """
    Container for debug information about template nodes.

    Used to track cell location and values for error reporting.

    Attributes:
        value: The cell value or content
        cell_tag: Associated CellTag if any
    """
...
class Node(object):
    """
    Base node class for the Excel template tree structure.

    Flow:
    1. Node is created and added to tree hierarchy
    2. enter() is called when node processing starts
    3. Children are processed recursively
    4. exit() is called when node processing completes

    Each node has:
    - Parent reference for traversing up the tree
    - Children list for traversing down the tree
    - Depth tracking for tree visualization
    - Unique key for Jinja2 template identification
    """
    ...
    def __init__(self):
        """Initialize node with empty children and no parent."""
        self._children = []
        self._parent = None
        self.no = 0
        self._depth = -1
    ...
    def tag_tree(self):
        ...
        Flow:
        1. Print current node with indentation based on depth
        2. Recursively print all children
        3. Creates visual tree representation
        """
        logger.debug("\t" * self.depth + " " + self.print_tag)
        for child in self._children:
            child.tag_tree()
